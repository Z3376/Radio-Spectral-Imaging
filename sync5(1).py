...
tbwp		=	a_t*i_n							#time-bandwidth-product
...
s_f_dev	=	np.zeros(i_n)

p_s_f_dev	=	np.zeros(i_n)

for i in range(i_n):
	for j in range(i):
		s_f_dev[i_n-j-1]	+=	s_f_dif
...
for i in range(i_n):
	if(i>0):
		f1.read(int(j1_o[i]-j1_o[i-1]))
		f2.read(int(j2_o[i]-j2_o[i-1]))
		j1	+=	int(j1_o[i]-j1_o[i-1])
		j2	+=	int(j2_o[i]-j2_o[i-1])
	for k in range(a_t):
		while(j1<(i+1)*(k+1)*1024*e_c):
			buff_1[np.abs(int(j1-j1_o[i]))%1024]	=	l_t[int(str(f1.read(1)).encode('hex'),16)]
			buff_2[np.abs(int(j2-j2_o[i]))%1024]	=	l_t[int(str(f2.read(1)).encode('hex'),16)]
			j1				+=	1
			j2				+=	1
			if(j2%1024==0):
				q2			+=	1
