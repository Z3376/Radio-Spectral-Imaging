...
X_A1		=   np.zeros((i_n,256),dtype='complex')
Y_A1		=   np.zeros((i_n,256),dtype='complex')
X_A2		=   np.zeros((i_n,256),dtype='complex')
Y_A2		=   np.zeros((i_n,256),dtype='complex')
...
for i in range(i_n):
	...
	while(j<1024*a_count):
		...
		if(j%1024==0):
			...
			#print p_num1[q%2],p_num2[q%2],p_num1[q%2]-p_num2[q%2]
			if(p_num1[q%2]-p_num2[q%2]!=off):
				print 'slip at:',p_num1[q%2],p_num2[q%2]
				while(p_num1[q%2]-p_num2[q%2]!=off):
					if(p_num1[q%2]-p_num2[q%2]>off):
						j	+=	1024
						f2.read(1052)
						p_num2[q%2]	=	int(str(f2.read(4)).encode('hex'),16)
...
fig21	=	plt.figure(21)
plt.subplot(221)
plt.imshow(amplitude(X_A1),interpolation='bilinear',aspect='auto')
plt.ylabel('X1')
plt.subplot(222)
plt.imshow(amplitude(X_A2),interpolation='bilinear',aspect='auto')
plt.ylabel('X2')
plt.subplot(223)
plt.imshow(amplitude(Y_A1),interpolation='bilinear',aspect='auto')
plt.ylabel('Y1')
plt.subplot(224)
plt.imshow(amplitude(Y_A2),interpolation='bilinear',aspect='auto')
plt.ylabel('Y2')
