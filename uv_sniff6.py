...
for das_id_1 in range(8):
	for das_id_2 in range(8):
		if(das_id_1!=das_id_2):
			...
			for i in range(l):
				X[ctr][i]	=	b*(np.cos(pi-gamma*d2r))
				Y[ctr][i]	=	-1*b*(np.sin(pi-gamma*d2r))
				Z[ctr][i]	=	b*(0)
				u[ctr][i]	=	(np.sin(ha[i]*d2r)*X[ctr][i])+(np.cos(ha[i]*d2r)*Y[ctr][i])
				v[ctr][i]	=	(-1*np.sin(dec*d2r)*np.cos(ha[i]*d2r))*X[ctr][i]+(np.sin(dec*d2r)*np.sin(ha[i]*d2r)*Y[ctr][i])+(np.cos(dec*d2r)*Z[ctr][i])
				w[ctr][i]	=	(np.cos(dec*d2r)*np.cos(ha[i]*d2r)*X[ctr][i])-(np.cos(dec*d2r)*np.sin(ha[i]*d2r)*Y[ctr][i])+(np.sin(dec*d2r)*Z[ctr][i])
