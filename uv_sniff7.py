...
min_d	=	0
min_j	=	[0 for i in range(l)]
for i in range(l):
	for j in range(len(ss_ZA)):
		d	=	np.abs(ZA[i]-ss_ZA[j])
		if(j==0):
			min_d	=	d
			min_j[i]	=	0
		if(d<min_d):
...
min_jf	=	[0 for i in range(l)]
di		=	0
for i in range(l):
	c	=	0
	for j in range(len(ss[1])):
		if(ss[1][j]==ss_ZA[min_j[i]]):
			df	=	((ZA[i]-ss[1][j])**2)+((Azm[i]-ss[0][j])**2)
			if(c==0):
				di	=	df
				min_jf[i]	=	j
			if(df<di):
