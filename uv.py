...
for i in range(l):
	...
	if(np.sin(ha[i]*d2r)>0):
		Azm[i]	=	360-Azm[i]
	u[i]	=	baseline*np.sin(Alt[i]*d2r)*np.sin(pi-(Azm[i]+gamma)*d2r)/c
	v[i]	=	-1*baseline*np.sin(Alt[i]*d2r)*np.cos(pi-(Azm[i]+gamma)*d2r)/c
	w[i]	=	baseline*np.cos(Alt[i]*d2r)*np.cos(pi-(Azm[i]+gamma)*d2r)/c
